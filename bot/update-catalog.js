...
const newContent = `import TelegramBot from 'node-telegram-bot-api';
import { CatalogService } from '../services/catalog.service';

const catalogService = new CatalogService();

export const catalogHandler = async (bot: TelegramBot, msg: TelegramBot.Message) => {
  const chatId = msg.chat.id;
  await showCatalogMenu(bot, chatId);
};

export const showCatalogMenu = async (bot: TelegramBot, chatId: number) => {
...
export const showMovieDetails = async (bot: TelegramBot, chatId: number, movieId: string) => {
  try {
...
export const handleCatalogCallback = async (bot: TelegramBot, callbackQuery: TelegramBot.CallbackQuery) => {
  ...
  try {
    if (data === 'catalog_menu') {
      ...
      await showMoviesList(bot, chatId, category, page);
    } else if (data.startsWith('movie_details_')) {
      const movieId = data.replace('movie_details_', '');
      await showMovieDetails(bot, chatId, movieId);
...
const startPurchaseFlow = async (bot: TelegramBot, chatId: number, movieId: string) => {
  try {
    const movie = await catalogService.fetchMovieById(movieId);
...
const showTrailer = async (bot: TelegramBot, chatId: number, movieId: string) => {
  try {
    const movie = await catalogService.fetchMovieById(movieId);
...
const toggleFavorite = async (bot: TelegramBot, chatId: number, movieId: string) => {
  try {
    const movie = await catalogService.fetchMovieById(movieId);
...
fs.writeFileSync(path.join(__dirname, 'src', 'handlers', 'catalog.handler.ts'), newContent, 'utf8');
console.log('[OK] catalog.handler.ts updated with real API integration');
