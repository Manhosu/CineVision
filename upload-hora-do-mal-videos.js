...
async function uploadVideoInParts(filePath, presignedUrls) {
  ...
  try {
    for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
      ...
      console.log(`   ✅ Parte ${partNumber}/${totalParts} concluída (${progress}%) - ${(bytesRead / (1024 * 1024)).toFixed(2)} MB`);
    }
...
async function uploadVideo(video) {
  ...
  try {
    ...
    const { uploadId, presignedUrls } = await initiateMultipartUpload(video.languageId, video.path);
